...
| Retire benchmark v2: its packs and cards describe other cells for 147 of its 163 bench ids (a rebuild kept files across a changed sample; the builder now rebuilds a moved bench id), and the staged arm's v2 run read those cards beside live tools on the right cells. Re-run that arm on v3, whose cells, key and cards match v1's, and take the v2 table off the Eval page | the re-run is paid, and the Eval page change waits for its replacement | approval of the re-run |
| Run the information ladder on benchmark v3: `d1` and `d2`, two more seeds of `d1`, and `d1` voted at `d2`'s compute, beside the extended model | built and tested with scripted backends; every arm is a paid run | approval of the runs |
...
| The Eval page's extended-model table and the v3 benchmark table | the extended model is computed; its table waits for the v3 arms so both are shown together | the runs above |
| **Interface and tour** | | |
