...
| ~~`hole_crosscheck` (the extraction crosscheck over hole positions, renamed in §E.3)~~: done 2026-09-21 as an MCP tool (`legacy_reader.mcp.holes`): the crosscheck outputs of the files whose holes sit within a radius of the cell, or one file, or one hole, one row per provincial match with the offset and bearing under the ids the crosscheck gave them, the datum-shift signature and the adjudication flag; dashboard sessions only, and not in the public-safe build | | done |
...
`label_context`, `nearby`, `coverage`, `crosscheck`, `retrieve`) are served over MCP by `legacy_reader.mcp`
