...
| Four more samples of `d1` on benchmark v2 (`ue arm run --arm d1 --sample 1..4`): the run-to-run spread of a single call, and the five-sample vote that gives the single call the staged agent's compute, the control the `d2`-against-`d1-vote5` comparison needs | deferred after the 2x2 arms ran; about $170 of CLI accounting | a decision to run them |
| The evidence readers (`d2`) given the geology scores out of fold (criteria, learned, extended; never the effort score), read against the plain average of the reader's and the extended model's rankings (0.531 on v2), which it must beat to show the reader adds more than arithmetic | the two rank cells differently (rank correlation 0.12) and averaging them gains only 0.014; one run | the effort score hidden from the arm's score tool, then one run |
| A comparison ranker: the reader asked which of two or a handful of cells is more prospective, aggregated into a ranking, in place of one probability per cell | the pointwise probability is coarse (29 distinct values over 114 cells) and the readers call almost every drilled cell a closer look | a research pass on pairwise and listwise LLM ranking, then one arm |
| Labelled examples in the reader's prompt, drawn only from the other spatial folds (drilled positives and drilled negatives) | the readers get 2 to 5% of drilled negatives right: they have never been shown what barren drilled ground looks like here | the same research pass: example selection without leakage |
| Higher reasoning effort for the readers | the weak point is telling barren drilled ground apart and ranking finely, not depth of reasoning; 2 to 3 times the cost | the two rows above showing headroom |
| A larger benchmark: more positives (the one-per-10-km thinning relaxed, the 33 held-out cells unsealed at the end) and a dated hindcast whose negatives are not 'drilled and unlabelled' | 114 cells cannot tell a gain under 0.10 PR-AUC from noise, which is the size of every improvement above | a benchmark v3 spec |
