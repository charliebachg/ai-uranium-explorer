...
**The prototype's acceptance story (stated 2026-09-21, before the benchmark runs):** one person opens the dashboard, sees the data and its readiness, inspects the model results on the Eval page and the analyst's chains on the enabled cells, talks to the interface agent about a cell, and has it call the analyst on that cell; every model call in that story runs on a cheap pay-per-token model, and the benchmark runs afterwards, once the story works end to end.
...
*Status 2026-09-21: built (Phase 4c, `uranium_explorer.interface`). The router is one structured call on a cheap
model (`z-ai/glm-5.3-flash` through OpenRouter, `UE_INTERFACE_MODEL`) over the eight kinds above plus `other`;
...
unmeasured, no arithmetic appears, and a "not met" node whose layer was never mapped or sampled around the
cell (`nearby`'s `in_footprint` is 0, from a footprint built in memory from the layer's own geometry) is
refused and told to record unknown, because absence of mapping is not absence of features (added
2026-09-21). A rejected node goes back to Stage 2 with feedback that escalates the
...
| Executor view | whole tables / segment-scoped (`segment_scoped`) | the template stages the whole feature and criteria tables for every segment; scoped, each executor is staged only its own criterion's rows (a cross-check the rows of the two it combines) and the gate holds its node to what it saw, so the arm measures what the ten-fold repetition costs in tokens and whether a narrower view changes what the executors cite |
...
checklist is a command, `ue prospect gate` (2026-09-19): one row per feature layer or scene, label layer and
...
| **Versioned** | a hashed pull that a value walks back to | every layer hashed and registered; store snapshots (`ue store snapshot`); each evaluation run names the snapshot it read |
...
|---|---|---|
| ~~**Langfuse** as the LLM observability layer~~: **out of scope for the prototype (decided 2026-09-21)**. The OpenTelemetry instrumentation and the OTLP exporter (§A.2) are what any such backend would consume; which backend, if any, is a deployment choice made when the prototype has more than one operator | its self-hosted stack is too heavy for the development machine and its datasets and scores duplicate the benchmark harness and MLflow | a deployment with an operator who wants it |
...
| ~~Segment-scoped tool views: each executor sees only its criterion's rows rather than the whole feature and criteria tables the template stages for every segment~~: built 2026-09-21, not yet run: the `segment_scoped` switch (§8.5) and the `v1-scoped` and `v1-scoped-batch` arms; the session stages each segment its own rows of the feature, criteria, coverage and cross-check tables and keeps a view per segment the gate holds its node to, the harness reads the whole `cell_features` once for itself, the private transcript and the leakage rules are untouched, and with the switch off the staged bundle and the executor prompts are byte-identical to before | the first v1 run has to show whether the ten-fold repetition costs anything but tokens | built, not yet run: the first v1 arm's cost per chain is on the table |
...
| ~~`hole_crosscheck` (the extraction crosscheck over hole positions, renamed in §E.3)~~: done 2026-09-21 as an MCP tool (`uranium_explorer.mcp.holes`): the crosscheck outputs of the files whose holes sit within a radius of the cell, or one file, or one hole, one row per provincial match with the offset and bearing under the ids the crosscheck gave them, the datum-shift signature and the adjudication flag; dashboard sessions only, and not in the public-safe build | | done |
...
| ~~**Absence of mapping is not absence of features**~~: done 2026-09-21, with no store, snapshot or feature change: each evidence layer's footprint is built in memory from its own geometry (`prospect.tools.layer_footprint`), the union of 5 km halos round every point or line and, for the map polygons, the union of the polygons themselves; halos rather than a concave hull because a hull fills the unsampled holes inside a survey and the gaps between survey blocks, and 5 km because regional lake-sediment surveys sample a lake every 2.4 to 3.6 km and one EM block's traces lie under 2 km apart, so 5 km joins one survey and leaves the next one's gap open (10 km bridges neighbouring blocks). `nearby` and `crosscheck` return `in_footprint` as a 0/1 value with an id and say in their note that an empty radius outside it is unknown; the node gate's sixth rule refuses a not-met node whose layer's flag is 0, names the layer and the cell, and tells the executor to record unknown, which may then cite the distance value. Cells outside the footprint, enabled (15) and benchmark v2 (163): EM conductors 2 and 18 (and 49% of the grid's 30,534 cells), faults 1 and 1 (9% of the grid), bedrock and surficial polygons 2 and 0, drillhole collars 2 and 14, lake sediment GSC 7 and 70 and SGS 6 and 82, lake water 12 and 127, radioactive boulders 8 and 96. The point-layer counts are the cells whose feature is already null, so nothing changes for them; the line-layer counts are the cells whose `d_conductor_m` and `d_fault_m` (5 to 59 km) the criteria score and the learned model still read as measurements, which is the data fix this row leaves open | | done |
...
| **As-published outliers in native layers**: the lake-water uranium field (`U_PPM`, as the province names it) has a median of 0.00005 ppm and a 99th percentile of 0.0038 ppm, and one site near an enabled cell reads 2.3 ppm, six hundred times the 99th percentile; the cheap verifier called the neighbouring sediment value (228 ppm, itself at the 99th percentile of a survey that reaches 882 ppm) a ppm-versus-ppb error, which it is not, but the suspicion fits the water value. The native tier carries values as published and never corrects them (§6); a range check that flags a native value as an outlier without changing it, shown beside the value and readable by the verifier, is the honest fix | found 2026-09-21 on the second on-demand chain; no validator exists for the native tier by design | Phase 5c, before the arms re-run, so the verifier reads a flag rather than guessing |
...
- **Seed pack (2026-09-21)**: `ue store seed pack|verify|unpack|ensure`. The store as one Parquet file per
  ...
  (2026-09-21)**: `ue store seed push <pack> --to s3://bucket/prefix` uploads a verified pack under
  `<prefix>/<hash>/`, its manifest last, then `<prefix>/manifest.json` as the pointer, so a partial push is
  never taken for a pack; `ue store seed pull <url>` (an `s3://` prefix, or an `https://` directory for a
  static host) reads the manifest, then every file with its sha256 checked, and refuses on any mismatch
  leaving nothing behind; `ensure` pulls from `UE_SEED_URL` when there is neither a store nor a pack on disk,
...
- **Traces (2026-09-21)**: the in-house spans (`spans.jsonl` per run, mirrored to MLflow Tracing) also export
  over OTLP/HTTP when `UE_OTLP_ENDPOINT` is set, same ids and attributes, batched and bounded (§A.2).
- **Auth and roles (done 2026-09-21)**: one key register for the API and the MCP server (`UE_MCP_KEYS`,
...
- **Background jobs (done 2026-09-21)**: a bounded thread pool inside the API process (DuckDB allows one
  ...
  (0.50 default, 2.00 at most) inside a per-process budget (`UE_JOB_SESSION_BUDGET_USD`, default 5), the chain
  published through the same `run_cells` as `ue arm chain`, the stage names into `progress_json` as the run's
...
- **Observability (decided 2026-09-20, built with 4a):** instrument with the **OpenTelemetry API** (every
  ...
  configuration**: with `UE_OTLP_ENDPOINT` set, every span is also exported over OTLP/HTTP (protobuf) with the
  file's own trace and span ids, the same attributes (run id, kind, cell, tool, value ids count, cost, latency,
  cache key, session id), resource `service.name=ai-uranium-explorer` and the pipeline version, from a bounded queue
  in batches, flushed at the end of each run, never blocking a model call and never raising; `UE_OTLP_HEADERS`
...
- A fresh clone reaches a running app in one command, with a seeded database, in under 15 minutes. **Met on
  ...
  verifying every table's hash and row count (§A.1). The pack now travels: `ue store seed push` publishes it
  to any S3 (the compose MinIO included) and `ue store seed pull <url>` fetches it from an `s3://` or
  `https://` URL with every hash checked, and the app's first start pulls from `UE_SEED_URL` by itself when
...
4 gaps recorded with evidence. Features are computed once by `ue prospect features`; there is no versioning
...
- Reproducible training: `ue prospect headline|modelsearch|hindcast --snapshot <hash>` refuses any store but
...
- CI runs the fold tests on every change to `prospect/models.py` and fails on a metric regression beyond the
  ...
  `UE_REAL_DATA=1` and a store exists; CI unpacks the public seed pack first (`ue store seed unpack`, §A.1),
...
1. **Deterministic** (~300 items): questions with exact answers computable from the store — "how far is the
   ...
   *Status 2026-09-21: built, not run. 299 items (255 answerable, 44 unanswerable) over the analyst
   ...
   `ue bench interface build`, hashed manifest, audit clean.*
...
*Status 2026-09-21: the loop and its scorer are built (Phase 4b, §12): `ue extract agent` runs locate → read →
validate → agree → file per page with second-family agreement into a review queue, and `ue gold key` writes
the skeleton a person fills while `ue gold score` reports precision and recall per field type against every
...
### E.1 Current state
Built 2026-09-21 (Phase 4a): the eight deterministic tools (`cell_features`, `cell_scores`, `criteria_breakdown`,
`label_context`, `nearby`, `coverage`, `crosscheck`, `retrieve`) are served over MCP by `uranium_explorer.mcp`
behind `open_session` handles, with `hole_crosscheck`, `check_claims`, `abstain`, `record_insight` (the
`expert` tier, migration 0005) and `run_analyst` as a stub; the seven resources and the six prompts; the gate
stamping every result into the session registry; a run manifest and a span per call; API-key scopes; streamable
HTTP on the API at `/mcp` and stdio through `ue mcp serve --stdio`; contract tests through the SDK's in-memory
...
and the task are idempotent by handle. Resources: `ue://handbook`, `ue://criteria`, `ue://cell/{id}/evidence`,
`ue://cell/{id}/chains`, `ue://run/{id}/manifest`, `ue://readiness/gate`, `ue://reading/inventory`. Prompts:
...
Decided 2026-09-21: **no human adjudication** (§5d, §D.3.4) and **no hand-keyed gold** (§8.2, Tier 4), because the project has no geologist and no second person; the mechanical tiers, the gate suite and the second-family agreement rate stand in, each with its denominator stated. **No Langfuse** (§9.6): the OTLP exporter exists and the backend is a deployment choice.
