...
| **Cover mask on `graphitic_host`**: where the cell sits under basin cover (an interpolated unconformity depth, or `in_basin`), the 1:250k bedrock polygon names the cover sandstone, not the basement host, so the feature and the criterion should read unknown, not not met. Found by the v1 verifier on three of the first eleven enabled cells (2026-09-20); the criteria score carries the same error for every covered cell | changes a feature and the criteria baseline; needs a rebuild and a re-score before 5c compares arms against it | before 5c, with the readiness gate re-run |
| **Zero-filled counts read as measurements**: radioactive-boulder records at 0.0 cps are nulls in the source layer; `boulder_max_cps` should treat 0 as missing. Found by the v1 verifier (2026-09-20) | a layer-ingestion rule with a rebuild behind it | with the cover mask, before 5c |
