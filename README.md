...
*The shape of the judgement, drawn high level on purpose: the benchmark is built but its runs are pending. Dashed boxes are built and waiting on those runs; the amber box is the line no number crosses without a stored value behind it. The figure is generated by `docs/figures/make_evaluation_flow.py`.*
