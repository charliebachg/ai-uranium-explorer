...
def oof_scores(con: Any, cell_ids: list[str], fold_kind: str = FOLD_KIND) -> tuple[dict[str, dict[str, float]], str | None]:
    ...
    con.register("lr_bench_cells", pd.DataFrame({"cell_id": list(cell_ids)}))
    try:
        rows = con.execute(
            "select s.cell_id, s.model, avg(s.score) from derived.cell_score_oof s join lr_bench_cells b using (cell_id) "
            "where s.fold_kind = ? and s.score is not null group by 1, 2", [fold_kind]
        ).fetchall()
    finally:
        con.unregister("lr_bench_cells")
