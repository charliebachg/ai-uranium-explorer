...
reason), through the same `run_cells` that `lr arm chain` runs, so the chain lands in `agent.chain*` exactly
as an offline one does and the evidence panel lists it. A job has a budget in dollars; the process has a
budget across jobs (`LR_JOB_SESSION_BUDGET_USD`), and a submission that would take the total past it is
...
SESSION_BUDGET_VAR = "LR_JOB_SESSION_BUDGET_USD"
DEFAULT_SESSION_BUDGET_USD = 5.0
WORKERS_VAR = "LR_JOB_WORKERS"
DEFAULT_WORKERS = 2
...
def enabled_cells(path: Path | None = None) -> set[str]:
    """The enabled cells of PRD §9.3, read from the same file `lr arm chain --enabled` reads."""
    ...
    def _pool_started(self) -> ThreadPoolExecutor:
        with self._lock:
            if self._pool is None:
                self._pool = ThreadPoolExecutor(max_workers=max(1, self.workers), thread_name_prefix="lr-job")
            return self._pool
...
class SpanWatcher:
    ...
    def start(self, path: Path) -> None:
        if self._thread is not None:
            return
        self.path = path
        self._thread = threading.Thread(target=self._run, name="lr-job-spans", daemon=True)
        self._thread.start()
...
@dataclass
class AnalystKind:
    """The staged analyst over one enabled cell, as `lr arm chain --enabled` runs it, as a job. Every piece a
