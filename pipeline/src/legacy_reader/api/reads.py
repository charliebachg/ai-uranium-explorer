...
class EvidenceCache:
    ...
    def warm(self, cell_ids: list[str], log: Callable[[str], None] = lambda _m: None) -> threading.Thread:
        ...
        th = threading.Thread(target=run, name="lr-evidence-warm", daemon=True)
        th.start()
        return th
...
class Candidates:
    """PostGIS when LR_PG_DSN is set and answers; DuckDB otherwise, and again after a failure, retried later."""
    ...
    def __init__(self, dsn: str | None = None, duck: Callable[[int, str], list[dict[str, Any]]] | None = None,
        ...
        self.dsn = dsn if dsn is not None else os.environ.get("LR_PG_DSN", "").strip()
