"""`lr export-eval`: run statistics for the eval page, before any gold labels exist.
...
the run cost. The web page states that, and the `gold` field stays null until `lr score` writes a scorecard.
"""
...
def _reports() -> list[dict[str, Any]]:
    ...
    if not idx.is_file():
        raise RuntimeError("no reports exported; run `lr export-reports` first")
