"""`lr export-reports`: write the per-report half of the web contract, gated by the contract checker.
...
def patch_manifest(target: Path, files_read: int, artifacts: dict[str, dict[str, Any]],
    ...
    if not path.is_file():
        log("  no manifest.json yet (run `lr export-web` first): the files_read stat was not patched")
        return False
...
def stage_export_reports(files: list[str] | None = None, public_safe: bool = False,
    ...
    stage = Path(tempfile.mkdtemp(prefix="lr_reports_", dir=PATHS.data))
