"""`lr extract agent` and `lr gold`: the extractor loop, and the hand-keyed pages it is scored against.
...
def agent_cmd(
    ...
    store: str = typer.Option(None, "--store", help="the DuckDB file the queue is filed in; default data/lr.duckdb"),
    ...
    `lr extract` are carried in as read, so the second family runs over them at no reader cost. Exits 3 on
