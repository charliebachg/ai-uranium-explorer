...
OpenTelemetry SDK, so the backend is a configuration and not a code change (PRD §A.2). With `LR_OTLP_ENDPOINT`
...
attributes, the resource `service.name=legacy-reader` and the pipeline version, and is exported over OTLP/HTTP
...
either way. `LR_OTLP_HEADERS` carries an auth header for a hosted backend; its value is never logged.
"""
...
def endpoint() -> str | None:
    """The traces URL from `LR_OTLP_ENDPOINT`, with `/v1/traces` added when only a host is given; None when unset."""
    raw = os.environ.get("LR_OTLP_ENDPOINT", "").strip()
    if not raw:
        return None
    parts = urlsplit(raw)
    if parts.path in ("", "/"):
...
def headers() -> dict[str, str]:
    """`LR_OTLP_HEADERS` as `name=value,name2=value2` (percent-encoded values allowed, as the OTel env form is)."""
    out: dict[str, str] = {}
    for item in os.environ.get("LR_OTLP_HEADERS", "").split(","):
        if "=" not in item:
            continue
...
class Bridge:
    ...
    def __init__(self, provider: Any, ids: Any, where: str) -> None:
        ...
        self.tracer = provider.get_tracer("legacy_reader", __version__)
        self.ids = ids
        self.where = where
...
def bridge(log: Callable[[str], None] = print) -> Bridge | None:
    """The process's bridge, built on first use when `LR_OTLP_ENDPOINT` is set (or one was installed); else None.
