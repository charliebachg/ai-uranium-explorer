...
def append_frame(
    ...
    con.register("lr_append_frame", df)
    con.execute(
        f"insert into {schema}.{table} ({', '.join(cols)}, tier) "
        f"select {', '.join(cols)}, '{tier}' from lr_append_frame"
    )
    con.unregister("lr_append_frame")
    return len(df)
...
def tier_audit(con: duckdb.DuckDBPyConnection) -> list[str]:
    ...
    Returns human-readable problems; an empty list means the store is clean. `lr store audit` prints these and
