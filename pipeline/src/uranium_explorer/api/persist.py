...
def save_conversation(conversation_id: str, cell_id: str, model: str, backend: str, db_path: Path | None = None,
                      requested_by: str | None = None) -> None:
    con = connect(db_path)
    try:
        con.execute(
            "insert into agent.conversation (conversation_id, cell_id, model, backend, created_at, requested_by) "
            "select ?, ?, ?, ?, ?, ? where not exists (select 1 from agent.conversation where conversation_id = ?)",
            [conversation_id, cell_id, model, backend, _now(), requested_by, conversation_id],
        )
    finally:
        con.close()
...
def save_turn(conversation_id: str, step: int, turn: dict[str, Any], tool_calls: list[dict[str, Any]],
              values: dict[str, Any], duration_s: float, db_path: Path | None = None,
    ...
    try:
        con.execute(
            ...
            "problems_json, claims_json, tool_calls_json, values_json, cost_usd, duration_s, created_at, requested_by) "
            "values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
...
def load_conversation(conversation_id: str, db_path: Path | None = None) -> dict[str, Any] | None:
    ...
    try:
        head = con.execute("select conversation_id, cell_id, model, backend, created_at, requested_by "
                           "from agent.conversation where conversation_id = ?", [conversation_id]).fetchone()
        if not head:
            return None
        rows = con.execute(
            "select step, question, answer_text, published, problems_json, claims_json, tool_calls_json, values_json, "
            "cost_usd, duration_s, created_at, requested_by from agent.conversation_turn where conversation_id = ? "
            "order by step", [conversation_id]).fetchall()
    finally:
        con.close()
    return {
