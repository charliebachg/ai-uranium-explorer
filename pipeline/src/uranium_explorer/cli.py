"""`ue` command line. Stages are added here as they are built."""
...
app = typer.Typer(no_args_is_help=True, add_completion=False, help="AI Uranium Explorer pipeline")
...
def enable_files_cmd(
    ...
    typer.echo(f"enabled {len(out)} file(s); next: ue fetch --only <num>, then render, ocr, route, extract --dry-run")
...
def extract_cmd(
    ...
    if ctx.invoked_subcommand is not None:
        return   # `ue extract agent ...`: the loop runs, not the batch reader
...
def locate_cmd(files: list[str] = typer.Option(None, "--files")) -> None:
    ...
    Locating runs inside `ue assemble` (a value and its box are made together); this command re-runs it
...
def store_rebuild_cmd() -> None:
    """Flatten every stage into the tiered data/ue.duckdb plus the two GeoParquet layers."""
...
def store_migrate_cmd(dsn: str = typer.Option(None, "--dsn", help="default: UE_PG_DSN or the compose default")) -> None:
...
def store_seed_unpack_cmd(
    ...
    into: str = typer.Option(None, "--into", help="the store file to create; default data/ue.duckdb. Refuses an existing file"),
) -> None:
...
def store_seed_push_cmd(
    src: str = typer.Argument(..., help="the pack directory, or its manifest.json"),
    to: str = typer.Option(..., "--to", help="s3://bucket/prefix; endpoint from UE_S3_ENDPOINT, credentials from the AWS env names"),
) -> None:
...
def store_seed_ensure_cmd() -> None:
    """The container's first step: when data/ue.duckdb is missing, unpack UE_SEED_DIR (default data/seed/latest), or pull UE_SEED_URL first; otherwise nothing."""
