...
def export(public_safe: bool = False, log: Any = print) -> Path:
    ...
    if missing:
        raise RuntimeError(f"missing pulls {missing}; run `ue index pull`")
    grid_path = target / "context" / "datum_grid.json"
    if not grid_path.is_file():
        raise RuntimeError("datum grid missing; run `ue crs shift-grid`")
    datum_grid = json.loads(grid_path.read_text())

    stage = Path(tempfile.mkdtemp(prefix="ue_export_", dir=PATHS.data))
