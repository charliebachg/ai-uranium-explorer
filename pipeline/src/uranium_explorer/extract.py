"""`ue extract`: the scheduler. One routed page per `claude -p` call, paced, capped, cached, resumable.
...
class Scheduler:
    ...
    def run(self, plan: list[PlannedPage]) -> dict[str, Any]:
        ...
        with ThreadPoolExecutor(max_workers=self.workers, thread_name_prefix="ue-extract") as pool:
        ...
        if summary["pending"]:
            ...
            self.runlog.state({
                ...
                "resume": f"ue extract --config {self.config.id}",
            })
        return summary
...
def stage_extract(config_id: str = "phase2", split: str = "dev", files: list[str] | None = None,
    ...
    if summary["usage_limit"]:
        ...
        log(f"resume with: ue extract --config {config.id}")
    return summary
