"""`ue mcp`: serve the contract, list it, mint a key."""
...
def serve_cmd(
    ...
    ue = build(public_safe=public_safe or None, runs_dir=Path(runs_dir) if runs_dir else None)
    if stdio:
        serve_stdio(ue)
    else:
        serve_http(ue, host=host, port=port, log=typer.echo)
