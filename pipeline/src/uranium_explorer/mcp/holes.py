"""`hole_crosscheck`: the extraction crosscheck (`uranium_explorer.crosscheck`) served as a tool.

`ue crosscheck` writes one file per assessment report under `data/out/crosscheck/<file>.json`: every extracted
...
def crosschecked_files() -> list[str]:
    """Every file `ue crosscheck` has written, sorted so the rows come out in one order."""
    d = crosscheck_dir()
...
def hole_crosscheck(*, centre: tuple[float, float] | None, cell_shown: str, hole_id: str | None = None,
    ...
    if file_num:
        if not crosscheck_path(file_num).is_file():
            raise HoleError(f"no crosscheck for file {file_num}: it has not been read, or `ue crosscheck --files "
