...
def footer(cell_id: str, session_id: str) -> str:
    return (f"You work through the MCP tools of this server with session_id {session_id}, opened on cell {cell_id}. "
            ...
            "nearby, coverage, crosscheck, retrieve); the handbook is the resource ue://handbook and the criteria "
            "table ue://criteria. Every number you state must cite, in its claim's value_ids, the id the tool "
...
def text_for(name: str, cell_id: str, session_id: str) -> str:
    ...
    if name == "analyst.verifier":
        return "\n\n".join([
            AP.verifier_system(),
            AP.verifier_user(f"(read the chain from the resource ue://cell/{cell_id}/chains, one node per line)",
