...
`ue://run/{id}/manifest` a run's manifest (`runtime.manifest.Manifest`, an MCP session's included);
`ue://readiness/gate` the five-column verdict `ue prospect gate` last wrote; `ue://reading/inventory` the
...
RESOURCES: list[types.Resource] = [
    types.Resource(uri="ue://handbook", name="handbook", title="The handbook",
    ...
    types.Resource(uri="ue://criteria", name="criteria", title="The criteria table",
    ...
    types.Resource(uri="ue://readiness/gate", name="readiness_gate", title="The data readiness gate",
                   description="The five-column verdict (present, licensed, covers, servable, versioned) per "
                               "dataset, as `ue prospect gate` last scored it.", mimeType="application/json"),
    types.Resource(uri="ue://reading/inventory", name="reading_inventory", title="The data inventory",
...
TEMPLATES: list[types.ResourceTemplate] = [
    types.ResourceTemplate(uriTemplate="ue://cell/{id}/evidence", name="cell_evidence", title="A cell's evidence record",
                           ...
                           mimeType="application/json"),
    types.ResourceTemplate(uriTemplate="ue://cell/{id}/chains", name="cell_chains", title="A cell's analyst chains",
    ...
    types.ResourceTemplate(uriTemplate="ue://run/{id}/manifest", name="run_manifest", title="A run manifest",
...
_CELL_URI = re.compile(r"^ue://cell/([^/]+)/(evidence|chains)$")
_RUN_URI = re.compile(r"^ue://run/([^/]+)/manifest$")
...
def read(uri: str, *, runs_dir: Path | None = None) -> types.TextResourceContents:
    ...
    if uri == "ue://readiness/gate":
        path = gate_path()
        if not path.is_file():
            raise _not_found(uri, "no gate verdict on disk; run `ue prospect gate` first")
        return types.TextResourceContents(uri=uri, mimeType="application/json", text=path.read_text())
    if uri == "ue://reading/inventory":
