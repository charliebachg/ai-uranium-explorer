...
SERVER_NAME = "ai-uranium-explorer"
TITLE = "AI Uranium Explorer: the evidence record over MCP"
...
INSTRUCTIONS = (
    ...
    "answer. The handbook is ue://handbook and the criteria table ue://criteria. Scored and benchmark sessions "
...
class UeServer:
    ...
    def principal(self, ctx: Any) -> Principal | None:
        ...
        local principal (or the key in `UE_MCP_KEY`). `ctx.request` is the Starlette request over HTTP and None
...
def build(**kw: Any) -> UeServer:
    """The server the transports and the tests run; keyword arguments as `UeServer` takes them."""
    return UeServer(**kw)


__all__ = ["INSTRUCTIONS", "LIST_TTL_MS", "PUBLIC_VAR", "SERVER_NAME", "TOOLS", "UeServer", "build"]
