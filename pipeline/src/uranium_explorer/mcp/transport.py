...
def security_for(ue: UeServer) -> TransportSecuritySettings | None:
    if ue.keyring.configured:
        return None
...
def session_manager(ue: UeServer) -> StreamableHTTPSessionManager:
    return StreamableHTTPSessionManager(app=ue.server, json_response=True, stateless=True,
                                        security_settings=security_for(ue))
...
class Endpoint:
    ...
    def __init__(self, ue: UeServer) -> None:
        self.ue = ue
        self.manager: StreamableHTTPSessionManager | None = None

    @asynccontextmanager
    async def running(self) -> AsyncIterator[None]:
        manager = session_manager(self.ue)
...
def mount(app: Any, ue: UeServer, path: str = MCP_PATH) -> Endpoint:
...
def http_app(ue: UeServer, path: str = MCP_PATH) -> Starlette:
    """A Starlette app of one route, for `ue mcp serve --http` when the API process is not running."""
    endpoint = Endpoint(ue)
...
def serve_http(ue: UeServer, host: str = "127.0.0.1", port: int = 8788, path: str = MCP_PATH,
               log: Callable[[str], None] = print) -> None:
    import uvicorn

    log(f"  MCP over streamable HTTP at http://{host}:{port}{path}  (tool contract {ue.server.version}"
        f"{', public-safe' if ue.public_safe else ''}, "
        f"{'keys from the register' if ue.keyring.configured else 'local clients only'})")
    uvicorn.run(http_app(ue, path), host=host, port=port, log_level="warning")


async def run_stdio(ue: UeServer) -> None:
    ...
    async with stdio_server() as (read_stream, write_stream):
        try:
            await ue.server.run(read_stream, write_stream, ue.server.create_initialization_options())
        finally:
            ue.close()


def serve_stdio(ue: UeServer) -> None:
    anyio.run(run_stdio, ue)
