"""`ue prospect corpus`: a searchable corpus of assessment reports, built without a single model call.
...
def linked_files(log: Callable[[str], None] = print) -> pd.DataFrame:
    ...
    if not holes_path.is_file() or not index_path.is_file():
        raise RuntimeError("run `ue index pull` first (geods_holes, file_index_uranium)")
