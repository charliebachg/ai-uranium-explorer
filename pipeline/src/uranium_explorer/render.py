"""`ue render`: page images for the Phase 1 dev files, and the pages table every later stage extends.
...
def guard_not_heldout(file_num: str, pdf_sha256: str, lock_path: Path | None = None) -> None:
    ...
    if not lock_path.is_file():
        raise HeldOutError(f"{lock_path} missing: lock the held-out split (`ue lock-heldout`) before rendering")
...
def _render_range(pdf: Path, first: int, last: int, out_dir: Path, dpi: int) -> dict[int, Path]:
    ...
    tmp = Path(tempfile.mkdtemp(prefix="ue_render_", dir=out_dir))
    try:
...
def phase1_documents() -> list[dict[str, Any]]:
    ...
    if not sel.get("locked"):
        raise HeldOutError("the split is not locked; run `ue lock-heldout` first")
    ...
    for n in selected_files(sel, phase1_only=True):
        if n not in docs:
            if n in enabled:
                ...
                # `ue render` picks it up once its documents are on disk
                continue
            raise FileNotFoundError(f"{n} has no fetched documents; run `ue fetch`")
        out.extend(docs[n])
    return out
