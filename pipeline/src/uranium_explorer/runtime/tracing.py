...
When MLflow is importable and `UE_TRACING_MLFLOW` is not "0", the same spans are mirrored to MLflow Tracing
...
When `UE_OTLP_ENDPOINT` is set, the same spans are also exported over OTLP (`otlp.py`: an OpenTelemetry SDK
...
def _mlflow_module(log: Callable[[str], None]) -> Any:
    ...
    if os.environ.get("UE_TRACING_MLFLOW", "1").strip() == "0":
        return None
    try:
