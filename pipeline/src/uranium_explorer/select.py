"""`ue select shortlist | probe | final`: choose the 12 assessment files, split them, pick the Phase 1 four.
...
def read_selection(path: Path | None = None) -> dict[str, Any]:
    ...
    if not path.is_file():
        raise FileNotFoundError(f"{path} missing; run `ue select shortlist` first")
...
def stage_final(log: Callable[[str], None] = print) -> dict[str, Any]:
    sel = read_selection()
    if "probe" not in sel:
        raise RuntimeError("run `ue select probe` first")
...
def enabled_files(sel: dict[str, Any]) -> list[str]:
    """Files registered by `ue enable-files`: dev files outside the original shortlist, never held out."""
...
def selected_files(sel: dict[str, Any], phase1_only: bool = False) -> list[str]:
    ...
    final = sel.get("final")
    if not final:
        raise RuntimeError("run `ue select final` first")
    if phase1_only:
...
def stage_enable(file_nums: list[str], reason: str, log: Callable[[str], None] = print,
    ...
    """`ue enable-files`: probe each file's listing and register it as a dev file for fetch/render/extract."""
    from .arcgis import ArcGisClient
...
def post_fetch_check(sel: dict[str, Any], pdf_records: dict[str, dict[str, Any]],
    ...
    constraints = [
        ...
        asdict(ConstraintResult("min_nad27_or_no_datum",
                                ...
                                "likelihood from era prior plus text-layer hits; OCR hits refine it after `ue route`")),
    ]
    suggestions = []
...
def lock_split(sel: dict[str, Any], pdf_hashes: dict[str, list[dict[str, Any]]], lock_path: Path,
    ...
    if missing:
        raise RuntimeError(f"held-out files not fetched yet (no report PDF hashes): {missing}; run `ue fetch`")
